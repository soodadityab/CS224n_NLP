...
class TextGenEnv(gym.Env):
    ...
    # provides a reward calculation based on choice
...
# load finetuned model/tokenizer from disk
...
def train_ppo(model_name, prompt, user_choice):
    # gets model, tokenizer from disk
  ...
  # using Mlp Policy 
    ...
    # learning happens over 8000 timesteps
    ppo_model.learn(total_timesteps=8000)
    # saves PPO model to disk
    ppo_model.save("ppo_textgen")

    # save the RL fine-tuned model and tokenizer
    model.save_pretrained("./gpt2-ppo-finetuned")
    tokenizer.save_pretrained("./gpt2-ppo-finetuned")

    return ppo_model

# query user with a prompt and generate 2 completions
def query_user_main():
